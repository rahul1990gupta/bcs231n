...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            h, cache_hidden = rnn_forward(out, h0, Wx, Wh, b)
        if self.cell_type == 'lstm':
            h, cache_hidden = lstm_forward(out, h0, Wx, Wh, b)
        ...
        if self.cell_type == 'rnn':
            dout, dh0, dWx, dWh, db = rnn_backward(dh, cache_hidden)
        if self.cell_type == 'lstm':
            dout, dh0, dWx, dWh, db = lstm_backward(dh, cache_hidden)
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length-1):
            out_embed, cache_e = word_embedding_forward(captions[:,i], W_embed)
            if self.cell_type == 'rnn':
                h, cache_r = rnn_step_forward(out_embed, prev_h, Wx, Wh, b)
            if self.cell_type == 'lstm':     
                h, c, cache_l = lstm_step_forward(out_embed, prev_h, prev_c, Wx, Wh, b)
            h = h.reshape((h.shape[0],1,h.shape[1]))
            scores, cache_t = temporal_affine_forward(h, W_vocab, b_vocab)
            scores = scores.reshape((scores.shape[0],scores.shape[2]))
            best_scores = np.argmax(scores, axis=1)
            captions[:,i+1] = best_scores
            prev_h = h
            prev_h = np.squeeze(prev_h)
            if self.cell_type == 'lstm':
                prev_c = c
